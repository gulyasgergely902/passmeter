...
struct ContentView: View {
        ...
        NavigationStack {
            ZStack {
                ...
                .sheet(isPresented: $isShowingAddModal) {
                    AddPassView { title, start, expiry, hasEntryLimit, totalEntries, isNotificationEnabled, notificationOffsetDays in
                        addItem(
                            title: title,
                            startDate: start,
                            expiryDate: expiry,
                            ...
                            notificationOffsetDays: notificationOffsetDays
                        )
    ...
    private func addItem(title: String, startDate: Date, expiryDate: Date, hasEntryLimit: Bool = false, totalEntries: Int = 0, isNotificationEnabled: Bool = false, notificationOffsetDays: Int = 3) {
        print("AddItem")
        withAnimation {
            let newItem = Item(
                title: title,
            ...
            if isNotificationEnabled {
                NotificationManager.instance.scheduleNotification(for: newItem, offset: newItem.notificationOffsetDays)
            }
	...
	private func renewItem(itemToRenew: Item, startDate: Date, expiryDate: Date, entryCount: Int, isNotificationEnabled: Bool = false, notificationOffsetDays: Int = 3) {
        ...
        withAnimation{
            ...
            itemToRenew.notificationOffsetDays = notificationOffsetDays
            if isNotificationEnabled {
                NotificationManager.instance.scheduleNotification(for: itemToRenew, offset: notificationOffsetDays)
            }
